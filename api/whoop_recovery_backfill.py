"""
Backfill Whoop recovery records into MinIO bronze layer.

Writes to: s3://whoop-lakehouse/bronze/whoop/recovery/dt=YYYY-MM-DD/recovery_<ts>_pN.json

Usage:
    cd api
    python whoop_recovery_backfill.py

    # Or override date range via env vars:
    WHOOP_BACKFILL_START=2025-12-01 WHOOP_BACKFILL_END=2026-01-01 python whoop_recovery_backfill.py
"""
...
def whoop_get_recovery(
    ...
    r = requests.get(
        f"{WHOOP_V2_BASE}{RECOVERY_LIST_PATH}",
        headers=headers,
        params=params,
        timeout=60,
    )
...
def backfill_recovery_to_minio(start_date: str, end_date: str, sleep_s: float = 1.0):
    ...
    for d in daterange(start_d, end_d):
        ...
        while True:
            page += 1
            data = whoop_get_recovery(token, window_start, window_end, limit=25, next_token=next_token)
            ...
            key = f"{prefix}/recovery/dt={d.isoformat()}/recovery_{safe_ts}_p{page}.json"
...
if __name__ == "__main__":
    ...
    backfill_recovery_to_minio(start, end)
