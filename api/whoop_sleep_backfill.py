"""
Backfill Whoop sleep records into MinIO bronze layer.

Writes to: s3://whoop-lakehouse/bronze/whoop/sleep/dt=YYYY-MM-DD/sleep_<ts>_pN.json

Usage:
    cd api
    python whoop_sleep_backfill.py

    # Or override date range via env vars:
    WHOOP_BACKFILL_START=2025-12-01 WHOOP_BACKFILL_END=2026-01-01 python whoop_sleep_backfill.py
"""
...
SLEEP_LIST_PATH = "/activity/sleep"
...
def whoop_get_sleep(
    ...
    r = requests.get(
        f"{WHOOP_V2_BASE}{SLEEP_LIST_PATH}",
        headers=headers,
        params=params,
        timeout=60,
    )
...
def backfill_sleep_to_minio(start_date: str, end_date: str, sleep_s: float = 0.25):
    ...
    for d in daterange(start_d, end_d):
        ...
        while True:
            page += 1
            data = whoop_get_sleep(token, window_start, window_end, limit=25, next_token=next_token)
            ...
            key = f"{prefix}/sleep/dt={d.isoformat()}/sleep_{safe_ts}_p{page}.json"
...
if __name__ == "__main__":
    ...
    backfill_sleep_to_minio(start, end)
